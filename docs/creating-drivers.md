...
**What the runtime enforces.** Everything you declare about a parameter is checked before the command reaches the device, however the command was run — a panel button, a macro, a schedule, the REST API, or the AI assistant. A value outside `min`/`max`, of the wrong type, or failing a `pattern` is refused, and the reason names the parameter: `'route': 'output' must be at most 8, got 99`. **Required** works the same way — leave a required parameter out and the command is refused with `'route': 'output' is required` rather than sending something incomplete. Tick Required only when the command genuinely cannot run without the value; if you would rather the driver filled in something the user did not give, that is a **Default**, not a requirement.

**A blank counts as nothing, except on free text.** An empty box on a required number, port, option, or child parameter is refused exactly like an omitted one: a blank names no quantity and no endpoint, so sending it would only reach the device as an incomplete command. A required **text** parameter is the exception and keeps a blank as a real value, because an empty string is a thing a device can be told: clearing a display name means sending an empty name.
...
- `summary_fields`: Which fields show as columns in the Child Entities list (the rest stay in the expanded per-child view). These name **type-level** `state_variables` — the properties every child of the type has. Omit it and the list falls back to the first three fields you declared (the platform's reserved keys are skipped — the row draws those itself), which is what you want for a normal type and is a trap for a `dynamic` one: see the note under `dynamic` below. Naming a reserved key here explicitly is honoured, if you want the raw value as a column beside the mark.
...
Most devices report state only when polled, and some push updates on the connection the driver already holds — both of those need nothing special. A `push` block is for devices that deliver notifications on a **separate channel the platform must open**. Four types are supported: `multicast` (the device sends state-change frames to a multicast group that OpenAVC joins), `sse` (the device streams updates over a Server-Sent-Events endpoint on its HTTP API), `tcp_listener` (the device dials back to a TCP port on the OpenAVC server after a registration command tells it where), and `http_listener` (the device delivers notifications by posting HTTP requests — webhooks — to a callback URL OpenAVC assigns).
...
- `type` — `http_listener`. Nothing else to declare: OpenAVC builds the callback URL at connect time — the server address as the device can reach it, on the web port, with a path unique to the device.
- Tell the device where to post by sending its registration command from `on_connect`. The token `{push_callback_url}` substitutes the assigned URL into command bodies, paths, and headers:
...
commands:
  register_feedback:
    method: POST
    path: /api/register
    body: '{"url": "{push_callback_url}", "events": ["status"]}'
```
...
- Everything that arrives goes through the driver's normal `responses` rules — same `match`/`set` semantics as a polled reply, nothing new to learn. A multicast datagram or dial-back frame carrying several messages is split on the driver's `delimiter` first; an SSE event and an HTTP-listener request body dispatch whole, exactly like an HTTP response body (pair JSON payloads with `json: true` response rules).
- Multicast frames, dial-back connections, and HTTP-listener posts are accepted **only from the device's own address**, so two identical devices pushing the same way each update their own OpenAVC device. SSE needs no filtering — the stream rides the driver's own HTTP session, with its authentication and TLS settings.
- **Response dispatch applies only the first matching rule per body.** That bites with push: a device that pushes a small leaf-shaped body per change but is polled with one broad query returning many values will parse the push fine and drop all but one value on every poll. Poll at the same granularity the device pushes.
- A dropped SSE stream reconnects on its own with exponential backoff (1 s doubling to 30 s); a device reboot re-establishes the subscription without any user action. Dial-back and webhook devices open connections toward OpenAVC, so there is nothing to reconnect on this side — re-registration on reconnect (plus polling) covers a device that rebooted.
- Push supplements polling; it doesn't replace it. Keep your `polling` block as the baseline resync — if the network filters multicast, an event stream drops, or a device quietly stops posting, the device still works, just at poll speed.
...
async def _on_frame(self, frame: str) -> None:
    self.record_traffic("rx", frame)
    self.set_state("power", json.loads(frame).get("power") == "on")
```
...
**Other reasons that stop the retry loop.** `host_key_rejected`, `tls_cert_untrusted`, `invalid_config`, and `client_missing` are treated the same way, just less strictly: they get a couple of attempts (a device rebooting mid-scan can briefly present one) and then auto-reconnect stops, because nothing about retrying can clear them — someone has to change a setting, trust a certificate, or install a client. The device card says so, and Reconnect starts a fresh attempt.

**You never raise `write_stalled` yourself.** The platform raises it when a device keeps its connection open but stops reading it, so a send that can never land fails in seconds instead of parking the command forever. Your driver sees an ordinary `ConnectionError` from `transport.send()`, and the device goes offline and reconnects the way it does for any other drop — there is nothing to handle and nothing to declare.
...
If you use an AI coding assistant (Claude, ChatGPT, Copilot, etc.), point it at this page and at the `AGENTS.md` file in the [community driver repository](https://github.com/open-avc/openavc-drivers/blob/main/AGENTS.md). This page is the reference for the driver formats themselves; `AGENTS.md` covers what that repository expects of a submission — naming, layout, driver metadata, validation, worked examples, and the runtime behaviors that are easy to get wrong. Alongside it are two generated JSON Schemas, [`avcdriver.schema.json`](https://github.com/open-avc/openavc-drivers/blob/main/avcdriver.schema.json) and [`pythondriver.schema.json`](https://github.com/open-avc/openavc-drivers/blob/main/pythondriver.schema.json), which give an assistant the machine-readable field list. The repository also includes `scripts/build_index.py` (run with `--check` to validate without writing) that your assistant can use to verify its work — it validates the schema, regex patterns, and catalog consistency.
