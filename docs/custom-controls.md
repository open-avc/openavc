...
**Anything in `ui/` is readable by anyone who can reach the server.** A wall panel presents no password, so these files are served without one, and panel approval does not gate them: approval decides which panels connect and control the space, not who can read a file by name. Keep credentials, customer data and anything else you would not hand out of this folder.
