...
A **flow** is a page of connected **nodes**; a message (`msg`) travels along the wires, and each node reads or changes it. `msg.payload` is the message's main value and `msg.topic` a label for it. Nothing runs until you click **Deploy**. The **debug** node prints a message to the sidebar on the right; the **inject** node sends one when you click its button; the **catch** node receives the errors other nodes raise. Nodes are added from the palette on the left, and more are installed from the menu under **Manage palette**. Node-RED's own [getting started guide](https://nodered.org/docs/tutorials/first-flow) covers the rest in ten minutes.
...
1. In the **UI Builder**, give a button a **Set Variable** action: `var.request_source` = `laptop`. Bind a label's value to `var.status`.
...
3. The flow does whatever it does: reads the projector's power with a **state get** node, checks a room calendar, asks a building system.
4. A **command** node switches the projector's input. A **set variable** node writes `Showing laptop` to `var.status`.
5. The label on the panel updates. So does the same label on every other panel in the space.

The example flow that ships with the nodes (**Import › Examples › @open-avc/node-red-openavc › logic-engine**) is exactly this, built against the Conference Room starter project, with an inject node standing in for the button so you can try it before building the panel:
...
| **state get** | Reads what a key holds right now, when a message arrives. A pattern gives every matching key and value. | `topic` is the key, `payload` the value (or where you point it) |
...
| **macro** | Runs a macro and answers when it has finished, however long it takes, or cancels one that is running. | `payload.success`, `payload.error`, `payload.step_errors`, `payload.cancelled` |
...
Every node's help, the book icon in the editor, says what goes in on the message and what comes out. The **device**, **command**, **macro** and **key** fields fill their lists from the system, so you pick rather than type. That needs the server node deployed first; the dialog says so if it is not.

A command or macro that fails puts the reason on `payload.error` in the same words OpenAVC's own panel would show, and raises an error a **catch** node can pick up, so a flow can react or just log it. A macro that finished with a failed step (macros carry on past a failed step unless set to stop) answers `success: true` with the failures listed in `payload.step_errors`.

To send a command to every device in a group, run a macro with a **Group Command** step, or send the command once per device.
...
Inside the flow, every OpenAVC node shows the connection on its status dot, and a **status** node turns those changes into messages, so the flow itself can react when the system goes away or comes back.
...
- **A `macro` node answers "macro skipped".** The macro's own **overlap** or **cooldown** setting turned the start away because a run was already going or had just finished. That is the macro doing what it was told; the flow now knows.
- **An `event in` node shows "needs an API key for plugin.*".** Without a key the connection hears `custom.`, `ui.`, `device.`, `macro.` and `system.` events; add the key to the server node for the rest.
...
- **A request answers "Rate limit exceeded".** The connection sent more than 200 messages in a second, which a loop in the flow does. The request is refused, not queued; find the loop.
