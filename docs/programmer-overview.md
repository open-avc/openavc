...
- **Code**: Python scripting, driver editing, and the custom controls you write yourself
...
- **New**: start with a blank project. It starts empty: the previous project's scripts, assets, custom controls and custom themes are all cleared, and the backup taken just before has them if you need one back
...
**Export** in the Program header downloads the running project as a `.zip` that carries everything it needs to come up somewhere else: the project file, its scripts, any drivers and plugins it uses, its assets, custom themes, and any custom controls. Driver files include their installed simulator and discovery companions. Save first, because the bundle is built from the saved project. **Import** takes that `.zip` back, adds it to the library and opens it, and it still accepts a plain `.avc` file on its own. Save As and Duplicate also carry the project's custom themes.
