...
### compare(actual, operator, target=None) -> bool

Compare two values with the same operator semantics macros and triggers use. Handles operator aliases (`"=="`, `">="`, `"equals"`, ...) and coerces types the way condition evaluation does everywhere else — so `compare("75", "gte", 50)` is `True` even though the state value arrived as a string, and a hopeless type mismatch returns `False` instead of raising `TypeError`.

Operators: `eq`, `ne`, `gt`, `lt`, `gte`, `lte`, `truthy`, `falsy` (plus aliases). For `truthy`/`falsy` the `target` argument is ignored.

```python
if compare(state.get("var.volume"), "gte", 50):
    await devices.send("amp_main", "duck")
```

Scripts generated by **Convert to Script** use `compare()` for every macro condition so the script behaves exactly like the macro did.
