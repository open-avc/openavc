...
**Validation Rules:** In a variable's detail panel, you can set optional validation constraints. For number variables, set a min and/or max value. For string variables, define a list of allowed values (enum). When a value violates its validation rule (set via macro, script, or UI), a warning appears in the variable detail and System Log. Activity records the value change. Validation retains the written value; it does not block or clamp the set, so automation continues running.
