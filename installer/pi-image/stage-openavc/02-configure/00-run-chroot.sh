...
#
# set_password takes the password from the request file, because the stored one
# is a hash now. That is why the helper travels with the release rather than
# living only in this image: the previous stage installs it from
# installer/openavc-privileged-helper.sh, the single copy, and
# installer/update-helper.sh refreshes an already-flashed appliance from the
# same file. Re-assert the trust boundary here — root-owned, in a directory the
# service user cannot write.
