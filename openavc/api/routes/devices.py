...
async def list_missing_drivers() -> dict[str, Any]:
    ...
    from openavc.api.routes.drivers import COMMUNITY_REPO_URL
...
async def install_missing_drivers(body: InstallMissingDriversRequest) -> dict[str, Any]:
    ...
    from openavc.api.routes.drivers import COMMUNITY_REPO_URL, install_community_driver
    from openavc.discovery.community_index import CommunityIndexCache
...
async def invoke_device_action(
    ...
    from openavc.core.setup_actions import SetupActionInProgress
...
async def store_pending_settings(
    ...
    # may have coerced values, and persisting the exact runtime dict keeps the
    # reconciler's device compare convergent (a skewed copy would re-add the
    # device on the next device-section reconcile for a change that is
    # already live).
