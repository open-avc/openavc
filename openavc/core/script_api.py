"""
OpenAVC Script API — the names user scripts import from 'openavc'.
...
The names themselves reach `openavc` through that package's own lazy
`__getattr__` (see openavc/__init__.py) — this module is not published into
sys.modules under a second name.
"""
...
# Named for the module it is, like every other logger in the tree. It used to be
# the hand-written "openavc.script_api", which worked only because the log
# categoriser treated a bare "openavc" prefix as "user script" — the prefix the
...
class _MacroProxy:
    ...
    async def execute(self, macro_id: str) -> None:
        ...
        from openavc.core.macro_engine import active_call_chain
