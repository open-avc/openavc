...
export interface DriverUdpSend {
  ...
  payload?: string;
  /**
   * Send a Wake-on-LAN magic packet instead of a payload. Names the config
   * field holding the device's MAC address, so it can be typed in for a device
   * that has never connected; a state variable of the same name is read first
   * once the device reports one. The packet goes to the broadcast address and
   * directly to host.
   */
  magic_packet?: string;
  /**
