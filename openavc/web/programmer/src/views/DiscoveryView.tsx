...
import type { DeviceState } from "../api/discoveryClient";
    ...
    >
      ...
      {expanded && (
        ...
        >
          ...
          {/* "Why?" reveal, and the audit of this one device */}
          <div>
            <button
              type="button"
              onClick={() => useAuditStore.getState().openWizard({ address: device.ip })}
              title="Check this device in detail and write a report about it"
              style={{
                background: "none", border: "none", cursor: "pointer",
                color: "var(--text-muted)", padding: 0, marginRight: "var(--space-md)",
                display: "inline-flex", alignItems: "center", gap: 4,
                fontSize: "var(--font-size-xs)",
              }}
            >
              <ClipboardCheck size={12} /> Audit this device
            </button>
            <button
              type="button"
